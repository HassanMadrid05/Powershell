# ==============================================================================
# GESTION DE PARAMETROS Y AUTOELEVACION
# ==============================================================================
param(
    [switch]$Resumen,
    [switch]$TiempoReal,
...
if ($TiempoReal -and -not [string]::IsNullOrEmpty($Archivo)) {
    Write-Error "El modo -TiempoReal no se puede usar junto con el parametro -Archivo."
    exit
}

if (-not ([System.Security.Principal.WindowsPrincipal][System.Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([System.Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Warning "Se requieren privilegios de Administrador. Relanzando el script..."
    Start-Sleep -Seconds 1
    try {
        $scriptArgs = @()
        if ($PSBoundParameters['Resumen']) { $scriptArgs += '-Resumen' }
        if ($PSBoundParameters['TiempoReal']) { $scriptArgs += '-TiempoReal' }
        if ($PSBoundParameters['Archivo']) { $scriptArgs += '-Archivo', "`"$($Archivo)`"" }
        ...
        Write-Error "No se pudo relanzar como Administrador. Por favor, ejecute la consola como Administrador manualmente."
...
if ($TiempoReal) {
    ...
    Write-Host "Iniciando modo de monitorizacion en tiempo real (CON REDES). Presiona 'q' para salir." -ForegroundColor Green
    Start-Sleep -Seconds 2
    
    $compInfo = Get-ComputerInfo | Select-Object CsDNSHostName, CsTotalPhysicalMemory
    
    while ($true) {
        if ($Host.UI.RawUI.KeyAvailable -and ('q' -eq $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown').Character)) {
            Write-Host "`nSaliendo del modo de monitorizacion..." -ForegroundColor Yellow
            break
        }
        
        $processes = Get-CimInstance -Class Win32_Process | Select-Object ProcessId,Name,ExecutablePath,@{N="WS";E={$_.WorkingSetSize}}
        $sortedProcesses = $processes | Sort-Object -Property WS -Descending
        $top15ProcessesForAnalysis = $sortedProcesses | Select-Object -First 15
        
        Clear-Host
        Write-Host "--- MODO MONITOR (Actualizado: $(Get-Date -Format 'HH:mm:ss')) --- Refresca cada ${refreshRateSeconds}s --- Presiona 'q' para salir ---" -ForegroundColor Cyan
        
        $totalPerf = Get-CimInstance -ClassName Win32_PerfFormattedData_PerfOS_Processor | Where-Object { $_.Name -eq '_Total' }
        $memInfo = Get-CimInstance -ClassName Win32_OperatingSystem
        $memFree = [math]::Round($memInfo.FreePhysicalMemory / 1024)
        $memTotal = [math]::Round($compInfo.CsTotalPhysicalMemory / 1MB)
        $memUsed = $memTotal - $memFree
        $memPercentUsed = [math]::Round(($memUsed / $memTotal) * 100, 2)

        Write-Host "Uso General CPU: $($totalPerf.PercentProcessorTime)% | Memoria Usada: $memUsed MB / $memTotal MB ($memPercentUsed %)" -ForegroundColor White
        
        $forensicDataRT = Show-ForensicSummary -topProcesses $top15ProcessesForAnalysis -compInfo $compInfo
        $forensicDataRT | Format-Table -AutoSize -Wrap -Property Nombre, PID, 'Uso CPU Actual (%)', 'Uso Memoria (%)', 'Ruta del Ejecutable'
    ...
    # --- MODO SNAPSHOT (COMPLETO O RESUMEN) ---
    $transcriptActive = $false
    try {
        if (-not [string]::IsNullOrEmpty($Archivo)) {
            try { Start-Transcript -Path $Archivo -Force; $transcriptActive = $true }
            catch { Write-Error "Error al escribir en el archivo '$Archivo'."; exit }
        }
...
"@
        ...
        Write-Host "                       by Hassan Madrid (https://github.com/HassanMadrid05/Powershell) (V3)" -ForegroundColor White
        Write-Host

        if ($Resumen) { Write-Host "Modo Resumen Activado." -ForegroundColor Yellow }
        ...
        if ($transcriptActive) { Stop-Transcript; Write-Host "`nReporte guardado en '$Archivo'" -ForegroundColor Green }
    }
}